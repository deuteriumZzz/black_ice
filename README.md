...
- **Liveness**: [libs/black_ice_common/liveness.py](libs/black_ice_common/liveness.py)
  ...
  replay, не больше. (2) `LivenessOnnxEngine` — теперь с реальным обученным
  чекпоинтом, не только точкой подключения: [ml/models/minifasnet_v2.onnx](ml/models/minifasnet_v2.onnx)
  (1.7МБ, Apache 2.0) — ONNX-конвертация официальных весов
  MiniFASNetV2/Silent-Face-Anti-Spoofing от minivision-ai, SHA-256 сверен с
  задокументированным на источнике конвертации. `deepface`'s встроенный
  антиспуфинг по-прежнему не подошёл (тянет TensorFlow+Keras+pandas, ломает
  пин `numpy<2`/onnxruntime-only стек — подтверждено реальной попыткой
  установить, не предположение); эта модель — чистый onnxruntime, второй
  фреймворк не нужен. `LIVENESS_ONNX_MODEL_PATH` теперь по умолчанию
  указывает на неё — включение `LIVENESS_CHECK_ENABLED=true` без
  дополнительной настройки сразу использует обученную модель, не эвристику
  (пустая строка в `LIVENESS_ONNX_MODEL_PATH` форсирует эвристику обратно).

  **Реальный баг, пойманный только эмпирической проверкой, не чтением
  документации**: у выбранного ONNX-экспорта — 3-классовый выход
  (live/print-attack/replay-attack), и ДВА независимых источника разошлись в
  том, какой индекс — "живое лицо": model card на Hugging Face утверждал
  индекс 0, официальный репозиторий minivision-ai своим `label == 1: Real
  Face` подразумевал индекс 1. Оба оказались неверны именно для этого
  конкретного ONNX-графа: прогон 6 реальных лиц (`insightface`'s bundled
  `t1.jpg`) дал устойчивый индекс **2** (~99% softmax-массы на каждом лице).
  Используется он — `LivenessOnnxEngine.LIVE_CLASS_INDEX = 2`, с комментарием
  в коде "перепроверяй эмпирически, не верь README", если чекпоинт когда-то
  сменится. Отдельно поймана и починена вторая неточность в документации
  модели: заявлялось, что выход — уже softmax, по факту это сырые логиты
  (сумма выхода ≈0.001, далеко не 1) — софтмакс применяется в коде.

  Кроп тоже воспроизводит оригинальный алгоритм дословно (не "похожий"):
  масштаб 2.7× вокруг bbox лица из ПОЛНОГО кадра (не из уже обрезанного лица
  — модели нужен контекст вокруг), сверено построчно с
  `minivision-ai/Silent-Face-Anti-Spoofing/src/generate_patches.py`. BGR без
  конвертации в RGB — оригинальный пайплайн каналы не переставляет.

  Проводка плагина проверена и на вручную собранной игрушечной ONNX-модели
  (препроцессинг/инференс/софтмакс независимо от конкретного чекпоинта), и
  живьём на реальной модели против реальных лиц
  ([tests/unit/test_liveness.py](tests/unit/test_liveness.py)) — а также через
  настоящий `/identify`: реальное фото прошло liveness-проверку и дошло до
  матчинга. Сама проверка выключена по умолчанию
  (`LIVENESS_CHECK_ENABLED=false`) — у любого метода есть реальный процент
  ложных отказов на дешёвых вебкамерах, это отдельное решение от того, какой
  метод используется, если включить.
...
- **Template protection**: model-inversion — по-прежнему только осведомлённость
  (см. раздел Security), не реализация (cancelable biometrics и т.п.).
