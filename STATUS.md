...
- ✅ Liveness — было ⚠️ (эвристика + честный ONNX plug-in point без модели), теперь
  закрыто реальной обученной MiniFASNetV2 ([ml/models/minifasnet_v2.onnx](ml/models/minifasnet_v2.onnx),
  1.7МБ, Apache 2.0). Не тренировали сами — конвертированный экспорт официальных
  весов minivision-ai/Silent-Face-Anti-Spoofing, SHA-256 проверен и совпадает с
  задокументированным на источнике; deepface по-прежнему не подошёл (тащит
  TensorFlow/Keras/pandas, ломает numpy<2/onnxruntime-only стек), эта модель —
  чистый onnxruntime, без второго фреймворка. `LIVENESS_CHECK_ENABLED=true`
  теперь по умолчанию использует её, не эвристику
  ([libs/black_ice_common/config.py](libs/black_ice_common/config.py)).
  **Реальный баг, пойманный только эмпирической проверкой**: и README модели на
  Hugging Face, и код официального репозитория расходились по вопросу "какой
  индекс из 3-классового выхода = живое лицо" (один утверждал 0, другой — 1) —
  оба оказались неверны для этого конкретного ONNX-экспорта. Прогон 6 реальных
  лиц через модель показал устойчивый индекс **2** (~99% softmax-массы каждый
  раз) — используется он, задокументировано в
  [liveness.py](libs/black_ice_common/liveness.py) прямо как "не верь README,
  перепроверяй эмпирически при смене чекпоинта". Отдельно поймана и починена
  вторая неточность в README модели: выход — сырые логиты, не софтмакс
  (сумма выходов ≈0.001, не 1) — софтмакс применяется в коде, не в графе.
  Проверено вживую: реальный `/identify` с реальным bundled-весом против
  настоящей фотографии лица — прошёл liveness-проверку (не `LIVENESS CHECK
  FAILED`), дошёл до матчинга.
...
107/107 зелёных, против реальных моделей и реальной семантики Qdrant/Postgres, не моков.
...
- ✅ **Обученная антиспуфинг-модель** — было в списке выше, теперь сделано и
  вычеркнуто. Полное описание (провенанс весов, пойманное расхождение в
  индексации классов между двумя источниками, живая проверка) — см. раздел
  "Security" выше.
